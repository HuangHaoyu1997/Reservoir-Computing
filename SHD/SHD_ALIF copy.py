...
class SRNN(nn.Module):
    ...
    def forward(self, input, device='cuda'):
        ...
        hid1_mem = torch.rand(batch, config.hid).to(device) # hid1_mem = torch.zeros(batch, config.hid).uniform_(0, 0.1).to(device)
        hid1_spk = torch.zeros(batch, config.hid).to(device)
        
        hid2_mem = torch.rand(batch, config.hid).to(device) # hid2_mem = torch.zeros(batch, config.hid).uniform_(0, 0.1).to(device)
        hid2_spk = torch.zeros(batch, config.hid).to(device)
        
        hid3_mem = torch.rand(batch, config.hid).to(device) # hid2_mem = torch.zeros(batch, config.hid).uniform_(0, 0.1).to(device)
        hid3_spk = torch.zeros(batch, config.hid).to(device)
        
        out_mem = torch.rand(batch, config.output).to(device) # out_mem = torch.zeros(batch, config.output).uniform_(0, 0.1).to(device)
...
class RC(nn.Module):
    ...
    def forward(self, input, device='cuda'):
        ...
        for t in range(time_step):
            ...
            if t > 10: output += F.softmax(out_mem, dim=1)
...
def train(model, optimizer, criterion, num_epochs, train_loader, test_loader, device):
    ...
    for epoch in range(num_epochs):
        ...
        for i, (images, labels) in enumerate(train_loader):
            ...
            optimizer.zero_grad()
            outputs, _, _, _ = model(images.to(device), device)
        ...
        if ts_acc > best_accuracy and tr_acc > 85:
            torch.save(model, './model-label_smooth_0.1_' + str(ts_acc) + '-readout-2layer-v2-4ms.pth')
...
def test(model, dataloader, is_test=0):
    with torch.no_grad():
        correct, total = 0, 0
...
plot_errorbar(train_acc_log, test_acc_log, 'SHD_ALIF-label_smooth_0.1.pdf')
