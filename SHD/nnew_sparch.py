...
class SNN(nn.Module):
    ...
    def _init_layers(self):
        ...
        if config.use_readout_layer: num_hidden_layers = self.num_layers - 1
        else:                        num_hidden_layers = self.num_layers

        for i in range(num_hidden_layers):
            snn.append(globals()[snn_class](input_size=input_size, hidden_size=self.layer_sizes[i], batch_size=self.batch_size,))
...
class RadLIFLayer(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, batch_size,):
        ...
        self.batch_size = self.batch_size * (1 + config.bidirectional)
...
def load_shd_or_ssc(split, batch_size, shuffle=True, workers=0,):
    """
...
class Experiment:
    ...
    def init_dataset(self):
        ...
        self.valid_loader = load_shd_or_ssc(split="valid", batch_size=self.batch_size, shuffle=False,)
        if config.dataset_name == "ssc":
            self.test_loader = load_shd_or_ssc(split="test", batch_size=self.batch_size, shuffle=False,)
        if config.use_augm:
    ...
    def init_model(self):
        ...
        if config.use_pretrained_model:
            ...
            self.net = SNN(input_shape=input_shape, layer_sizes=layer_sizes,).to(config.device)
