...
class RC(nn.Module):
    def __init__(self):
        ...
        self.a.data = torch.clamp(pareto.sample(self.a.shape) - 1, min=0, max=1.0) * (self.a[1] - self.a[0]) + self.a[0]
        self.b.data = torch.clamp(pareto.sample(self.alpha.shape) - 1, min=0, max=1.0) * (self.alpha_lim[1] - self.alpha_lim[0]) + self.alpha_lim[0]
        self.alpha.data = torch.clamp(pareto.sample(self.alpha.shape) - 1, min=0, max=1.0) * (self.alpha_lim[1] - self.alpha_lim[0]) + self.alpha_lim[0]
