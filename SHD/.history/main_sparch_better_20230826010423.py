...
class config:
    ...
    seed = 117 # round(time.time())
    ckpt_freq = -1
    threshold = 1.0
    
    smoothing = 0.1
    pdrop = 0.1
    normalization = 'batchnorm' # layernorm 前期学的更慢,但收敛更快,最终水平差不多
    train_input = False
    nb_hiddens = 1024
    noise_test = 0.0
...
class RC(nn.Module):
    def __init__(self):
        ...
        self.alpha_init = nn.Parameter(torch.Tensor(128), requires_grad=True)
        nn.init.uniform_(self.alpha_init, self.alpha_lim[0], self.alpha_lim[1])
        
        self.alpha = self.alpha_init.unsqueeze(1).repeat(1, 8).view(-1)
        ...
        # self.alpha.data = torch.clamp(pareto.sample(self.alpha.shape) - 1, min=0, max=1.0) * (1.0-0.6) + 0.6
        # self.beta.data = torch.clamp(pareto.sample(self.beta.shape) - 1, min=0, max=1.0) * (self.beta_lim[1] - self.beta_lim[0]) + self.beta_lim[0]
        # self.a.data = torch.clamp(pareto.sample(self.a.shape) - 1, min=0, max=1.0) * (self.a_lim[1] - self.a_lim[0]) + self.a_lim[0]
        # self.b.data = torch.clamp(pareto.sample(self.b.shape) - 1, min=0, max=1.0) * (self.b_lim[1] - self.b_lim[0]) + self.b_lim[0]
        # self.alpha_read.data = torch.clamp(pareto.sample(self.alpha_read.shape) - 1, min=0, max=1.0) * (self.alpha_lim[1] - self.alpha_lim[0]) + self.alpha_lim[0]
    ...
    def radlif_cell(self, Wx, mask, alpha, beta, a, b, V):
        ut = torch.zeros(Wx.shape[0], Wx.shape[2]).to(Wx.device) # rand
        wt = torch.zeros(Wx.shape[0], Wx.shape[2]).to(Wx.device)
        st = torch.zeros(Wx.shape[0], Wx.shape[2]).to(Wx.device)
        s = []
        # Bound values of parameters to plausible ranges
        alpha_ = torch.clamp(self.alpha_init, min=self.alpha_lim[0], max=self.alpha_lim[1])
        alpha_ = torch.clamp(self.alpha, min=self.alpha_lim[0], max=self.alpha_lim[1])
        ...
        # print('before:', alpha__[0].item(), alpha__[8].item(), alpha_[0].item(), alpha_[1].item())
        # with torch.no_grad():
        alpha__ = alpha_.unsqueeze(1).repeat(1, 8).view(-1)
        print('after:', alpha__[0].item(), alpha__[8].item(), alpha_[0].item(), alpha_[1].item(),'\n')
        ...
        for t in range(Wx.shape[1]):
            ...
            wt = beta_ * wt + ut + st
            ut = alpha__ * (ut - st) + (1 - alpha__)*(Wx[:, t, :] + torch.matmul(st, V_) - wt) # (1 - alpha_) 可以去掉，换成一个常数，0.15左右
...
def train(trial:int, model:RC, optimizer, criterion, num_epochs, train_loader, test_loader, scheduler):
    logging.info(f'\n**************  Trial {trial+1}  **************')
    model.train()
    train_accs, test_accs = [], []
    mask = 0 # [m1.float().to(config.device), m2.float().to(config.device)]
    
    torch.save(model, config.save_dir+'/ckpt/before-train-{:d}-{:d}-{:.2f}.tar'.format(trial, 0, 0))
    cin = ((model.alpha.view(config.cluster, config.nb_hiddens//config.cluster) - 
            model.alpha.view(config.cluster, config.nb_hiddens//config.cluster).mean(0))**2).mean() # 簇内聚
    cout = model.alpha.view(config.cluster, config.nb_hiddens//config.cluster).mean(0).std() # 簇外散
    logging.info(f"Epoch {0}: cin|cout={cin:.6f}|{cout:.6f}|{cout/cin:.4f}")
    logging.info("-----------------------------\n")
    best_acc, best_epoch = config.best_acc, 0
    for e in range(1, num_epochs+1):
        ...
        for i, (samples, labels) in enumerate(train_loader):
            ...
            if config.smoothing>0:     # label smoothing
                with torch.no_grad():
            ...
            # for l in range(config.output_dim):
            #     l_index = torch.where(labels==l)[0]
            #     loss += -config.fr_ent*(all_spikes[l_index,:,:].mean(0) * (all_spikes[l_index,:,:].mean(0)+1e-5).log()).mean()
            # firing_entropy =  -(all_spikes.mean(0) * (all_spikes.mean(0)+1e-4).log()).mean()
            # loss += (-firing_entropy)*config.fr_ent
            
            # loss += ((model.alpha.view(8,128) - model.alpha.view(8,128).mean(0))**2).mean() # 簇内聚
            # loss -= model.alpha.view(8,128).mean(0).std() # 簇外散
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
        ...
        train_accs.append(tr_acc); test_accs.append(ts_acc)
        scheduler.step(ts_acc)
        elapsed = str(timedelta(seconds=time.time() - now))[5:]
        logging.info(f"Epoch {e}: loss={tr_loss:.4f}|{ts_loss:.4f}, acc={tr_acc:.4f}|{ts_acc:.4f}, fr={epoch_spike_rate:.4f}, cin|cout={cin:.6f}|{cout:.6f}|{cout/cin:.4f}, lr={current_lr:.4f}, time={elapsed}")
        if ts_acc > best_acc:
            best_acc = ts_acc; best_epoch = e
            torch.save(model, f"{config.save_dir}/ckpt/best_model_{trial}_{ts_acc}.pth")
            logging.info(f"\nBest model saved with valid acc={ts_acc}")
        if config.ckpt_freq>0 and e % config.ckpt_freq==0:
            torch.save(model, config.save_dir+'/ckpt/model-{:d}-{:d}-{:.2f}.tar'.format(trial, e, ts_acc))
        logging.info("-----------------------------\n")
...
def multiple_trial(train_loader, test_loader):
    ...
    os.makedirs(config.save_dir+'/ckpt') if not os.path.exists(config.save_dir+'/ckpt') else None
    ...
    for i in range(config.trials):
        ...
        #     {'params': model.alpha, 'lr': config.lr*1.0},
        #     {'params': model.beta, 'lr': config.lr*1.0},
        #     {'params': model.a, 'lr': config.lr*1.0},
        #     {'params': model.b, 'lr': config.lr*1.0},
        #     {'params': model.alpha_read, 'lr': config.lr*1.0},
