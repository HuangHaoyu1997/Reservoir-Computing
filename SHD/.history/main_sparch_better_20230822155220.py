...
def train(trial:int, model:RC, optimizer, criterion, num_epochs, train_loader, test_loader, scheduler):
    ...
    cin = ((model.alpha.view(config.cluster, config.nb_hiddens//config.cluster) - model.alpha.view(config.cluster, config.nb_hiddens//config.cluster).mean(0))**2).mean() # 簇内聚
