...
class RC(nn.Module):
    def __init__(self):
        ...
        samples = pareto.sample(self.alpha.shape) - 1 # 生成1000个样本
        samples -= 1
        samples = torch.clamp(samples, min=0, max=1.0)
        samples *= (self.alpha_lim[1] - self.alpha_lim[0])
        samples += self.alpha_lim[0]
