...
class RC(nn.Module):
    def __init__(self):
        ...
        A = torch.randn(2)  # 创建一个128维的随机向量
        A.requires_grad = True
        repeated_A = A.unsqueeze(1).repeat(1, 8).view(-1)
        B = repeated_A.repeat(16 // len(repeated_A))
        
        
        
        self.alpha = nn.Parameter(torch.Tensor(config.nb_hiddens))
        for i in range(128): self.alpha.data[i*8:(i+1)*8] = self.alpha_init[i]
