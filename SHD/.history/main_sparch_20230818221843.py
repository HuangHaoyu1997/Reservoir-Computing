...
def train(trial, model, optimizer, criterion, num_epochs, train_loader, test_loader, scheduler):
    ...
    for e in range(num_epochs):
        ...
        for i, (samples, labels) in enumerate(tqdm(train_loader)): # 
            ...
            samples = torch.sign(samples.clamp(min=0)).to(config.dev) # all pixels should be 0 or 1
            outputs, sum1_spk, sum2_spk, A_norm, cluster_in, cluster_out = model(samples, mask)
            outputs, firing_rates, all_spikes
        ...
        print('epoch:%d, Loss:%.4f, Tr Acc:%.4f, Ts Acc:%.2f, Time:%.4f,\tA Norm:%.4f,\tFr:%.4f, Mask:%.4f, Cin:%.4f, Cout:%.4f'%\
            (e, loss.item(), tr_acc, ts_acc, time.time()-now, A_norm.item(), firing_rate, (m1==0).sum().item()/config.hid**2, cluster_in.item(), cluster_out.item()))
