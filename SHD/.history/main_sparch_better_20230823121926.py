...
class RC(nn.Module):
    ...
    def radlif_cell(self, Wx, mask, alpha, beta, a, b, V):
        ...
        print('before:', self.alpha[0].item(), self.alpha[8].item(), self.alpha_init[0].item(), self.alpha_init[1].item())
        # with torch.no_grad():
        self.alpha = alpha_.unsqueeze(1).repeat(1, 8).view(-1)
        print('after:', self.alpha[0].item(), self.alpha[8].item(), self.alpha_init[0].item(), self.alpha_init[1].item(),'\n')
        ...
        for t in range(Wx.shape[1]):
            ...
            ut = alpha_ * (ut - st) + (1 - alpha_)*(Wx[:, t, :] + torch.matmul(st, V_) - wt) # (1 - alpha_) 可以去掉，换成一个常数，0.15左右
