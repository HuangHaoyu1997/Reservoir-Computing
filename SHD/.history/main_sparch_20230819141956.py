...
def train(trial, model, optimizer, criterion, num_epochs, train_loader, test_loader, scheduler):
    ...
    for e in range(num_epochs):
        ...
        logging.info(f"Epoch {e}: tr loss={tr_loss:.4f}, te loss={ts_loss:.4f}, acc={tr_acc:.4f}, fr={epoch_spike_rate:.4f}, lr={current_lr:.4f}, time={elapsed}")
        logging.info("\n-----------------------------\n")
...
if __name__ == "__main__":
    ...
    # train_loader = torch.utils.data.DataLoader(dataset=nmnist_train, batch_size=_, shuffle=True, drop_last=False, num_workers=0)
