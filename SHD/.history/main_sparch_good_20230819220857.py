...
def train(trial:int, model:RC, optimizer, criterion, num_epochs, train_loader, test_loader, scheduler):
    ...
    logging.info(f"Epoch {e}: loss={tr_loss:.4f}|{ts_loss:.4f}, acc={tr_acc:.4f}|{ts_acc:.4f}, fr={epoch_spike_rate:.4f}, cin|cout={cin:.6f}|{cout:.6f}")
    ...
    for e in range(1, num_epochs+1):
        ...
        logging.info(f"Epoch {e}: loss={tr_loss:.4f}|{ts_loss:.4f}, acc={tr_acc:.4f}|{ts_acc:.4f}, fr={epoch_spike_rate:.4f}, cin|cout={cin:.6f}|{cout:.6f}, lr={current_lr:.4f}, time={elapsed}")
        if ts_acc > best_acc:
