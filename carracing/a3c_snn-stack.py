...
class NNPolicy(nn.Module):
    ...
    def forward(self, input, mask):
        ...
        time_step, channel, height, width = input.shape
        batch = 1
        ...
        for t in range(time_step):
            ########## Layer 0 ##########
            x = F.relu(self.conv1(input[t].unsqueeze(0).float()))
...
def train(shared_model, shared_optimizer, rank, info, masks):
    ...
    while info['frames'][0] <= config.max_frame or config.test:
        ...
        for step in range(config.update_freq):
            episode_length += 1
            value, logit = model(torch.tensor(state_deque).view(config.time_step, 3, 96, 96), masks[int(info['frames'][0]//config.dropout_feq)])
