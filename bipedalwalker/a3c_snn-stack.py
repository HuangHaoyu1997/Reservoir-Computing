...
class NNPolicy(nn.Module):
    ...
    def forward(self, input, mask):
        ...
        # if len(input.shape)!=2: print(input.shape)
        time_step, _ = input.shape
        batch = 1
        ...
        self.hid1_hid1.weight.data = self.hid1_hid1.weight.data * mask.T.to(config.device)
        for t in range(time_step):
            ########## Layer 1 ##########
            inpt_hid1 = self.inpt_hid1(input[t,:].float()) + self.hid1_hid1(hid1_spk)
...
def train(shared_model, shared_optimizer, rank, info, masks):
    ...
    tmp = env.reset()[0] 
    tmp += np.random.random(tmp.shape) * 0.3
    ...
    while info['frames'][0] <= config.max_frame or config.test:
        ...
        for step in range(config.update_freq):
            episode_length += 1
            value, dist = model(torch.tensor(state_deque), masks[int(info['frames'][0]//config.dropout_feq)])
            action = dist.sample()
            ...
            state += np.random.random(state.shape) * 0.3
            ...
            if num_frames % config.dropout_feq == 0:
                print('apply mask', (masks[int(num_frames//config.dropout_feq)]==0).sum().item()/config.hid**2)
                # torch.manual_seed(config.seed)
                # mask1 = mask1&((torch.rand(config.hid, config.hid) > config.dropout_stepping).int() * (1-torch.eye(config.hid, config.hid)).int())
                # torch.manual_seed(config.seed + rank)
            # elif num_frames % 0.1e6 == 0:
            #     print('no apply mask', (mask1==0).sum().item()/config.hid**2)
        ...
        next_value = torch.zeros(1,1) if done else model(torch.tensor(state_deque), masks[int(info['frames'][0]//config.dropout_feq)])[0]
