...
class NNPolicy(nn.Module):
    ...
    def forward(self, input, mask):
        ...
        for _ in range(config.time_step):
            ########## Layer 1 ##########
            inpt_hid1 = self.inpt_hid1(input.float()) + self.hid1_hid1(hid1_spk)
...
def train(shared_model, shared_optimizer, rank, info, masks):
    ...
    while info['frames'][0] <= config.max_frame or config.test:
        ...
        for step in range(config.update_freq):
            episode_length += 1
            value, logit = model(state, masks[int(info['frames'][0]//config.dropout_feq)])
        ...
        next_value = torch.zeros(1,1) if done else model(state, masks[int(info['frames'][0]//config.dropout_feq)])[0]
